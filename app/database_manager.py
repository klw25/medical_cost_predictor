...
class DatabaseManager:
    ...
    def clear(self,bmiEntry, diabetesEntry, hypertensionEntry, heartDiseaseEntry, asthmaEntry, dailyStepsEntry, sleepHoursEntry, stressLevelsEntry, annualDoctorVisitsEntry, hospitalAdmissionsEntry, medicationCountEntry, insuranceCoverageEntry, previousYearCostEntry, predictedAnnualMedicalCostEntry, canvas, patient_id_text):
        ...
        canvas.itemconfigure(patient_id_text, text="")
    ...
    def update_screen(self,bmiEntry, diabetesEntry, hypertensionEntry, heartDiseaseEntry, asthmaEntry, dailyStepsEntry, sleepHoursEntry, stressLevelsEntry, annualDoctorVisitsEntry, hospitalAdmissionsEntry, medicationCountEntry, insuranceCoverageEntry, previousYearCostEntry, predictedAnnualMedicalCostEntry, canvas, patient_id_text):
        if self.mydb:
            mycursor = self.mydb.cursor()
            print(f"Current ID: {self.current_id}")
            mycursor.execute("SELECT * FROM medical_cost_data WHERE id = %s", (self.current_id,))
            self.clear(bmiEntry, diabetesEntry, hypertensionEntry, heartDiseaseEntry, asthmaEntry, dailyStepsEntry, sleepHoursEntry, stressLevelsEntry, annualDoctorVisitsEntry, hospitalAdmissionsEntry, medicationCountEntry, insuranceCoverageEntry, previousYearCostEntry, predictedAnnualMedicalCostEntry, canvas, patient_id_text)
            ...
            canvas.itemconfigure(patient_id_text, text=f"Patient {self.current_id} of {self.max_id()}")
    ...
    def close(self, bmiEntry, diabetesEntry, hypertensionEntry, heartDiseaseEntry, asthmaEntry, dailyStepsEntry, sleepHoursEntry, stressLevelsEntry, annualDoctorVisitsEntry, hospitalAdmissionsEntry, medicationCountEntry, insuranceCoverageEntry, previousYearCostEntry, predictedAnnualMedicalCostEntry, canvas, patient_id_text):
        if self.mydb:
